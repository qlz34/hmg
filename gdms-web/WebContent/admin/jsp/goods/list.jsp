...
function updateGtid(gtid,$select){
	...
	//alert("要修改的id="+goodsid+"类别id="+gtid);
	//var $tr = $(gtid).parents("tr");
	$.ajax({
 	...
 	$c.show();setTimeout(function(){$c.close().remove()},2000);
...
function dodelete(){
	...
	$.ajax({
		...
		success : function(r){
			...
			$c.show();
			setTimeout(function(){$c.close().remove()},2000);
